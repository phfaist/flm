...
optional_text_arg = LLMArgumentSpec(
    parser='[',
    argname='thmtitle',
    llm_doc='An optional theorem environment title',
)



class TheoremEnvironment(llmspecinfo.LLMEnvironmentSpecBase):
    ...
    def render(self, node, render_context):
        ...
        llmarg_thmtitle = node.llmarg_thmtitle
        llmarg_labels = node.llmarg_labels
        ...
        if self.theorem_type_spec['numbered']:
            ...
            title_heading_formatted_llm = ref_instance.formatted_ref_llm_text

            target_id = f'{self.environmentname}-{counter.value}'

            title_heading_formatted_llm_frag = render_context.make_standalone_fragment(
                title_heading_formatted_llm,
                what='Theorem heading',
            )
            title_heading_formatted_llm_frag_nodes = title_heading_formatted_llm_frag.nodes

            for ref_type, ref_label in llmarg_labels:
            ...
            title_heading_formatted_llm_frag_nodes = node.latex_walker.make_nodelist(
                [
        ...
        thmtitle_nodelist = None
        if llmarg_thmtitle['has_relation_ref']:
            # build the thmtitle node list from the relation ref, if applicable.
            if llmarg_thmtitle['relation_ref_show_ref']:

                ref_type, ref_label = llmarg_thmtitle['relation_ref_target']
                ...
                thm_relation_ref_llm_text = thm_relation_ref_instance.formatted_ref_llm_text
                thm_relation_ref_llm_frag = render_context.make_standalone_fragment(
                    thm_relation_ref_llm_text,
                    what='Thm relation ref'
                )
                thmtitle_nodelist = thm_relation_ref_llm_frag.nodes

        elif llmarg_thmtitle['nodelist'] is not None:

            thmtitle_nodelist = llmarg_thmtitle['nodelist']


        if thmtitle_nodelist is not None:

            heading_nodelist = node.latex_walker.make_nodelist(
                list(title_heading_formatted_llm_frag_nodes) +
                [
                    node.latex_walker.make_node(
                        latexnodes_nodes.LatexCharsNode,
            ...
            heading_nodelist = title_heading_formatted_llm_frag_nodes


        heading_node = make_invocable_node_instance(
            latexnodes_nodes.LatexMacroNode,
            llm_spec=headings.HeadingMacro(
                macroname=None,
        ...
        heading_node.llm_heading_target_id = target_id
        ...
        final_content_node.llm_strip_preceding_whitespace = True
